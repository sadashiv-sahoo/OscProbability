 ...
 *  @file PREM.cxx
 *
 *  @brief It generates the OscEarthPREMProfle.cxx in src directory.
 *
 *  @author
 *
 *  SADASHIV SAHOO <br>
     ...
     int main(void)
 {
       ...
       if(PREM_FILE.good())
     {
         ...
         if(is_sorted(PREM.begin(), PREM.end()))
      {
         ...
         outfile1 << "    void OscProbabilitySTD ::\n    SetEarthDensityProfile()\n  {" << endl;
         outfile1 << "    const size_t rows =  std::SizeOf(LOADED_PREM);" << endl;
         outfile1 << "    PREM.clear();" << endl;
         outfile1 << "    for(size_t i = 0; i < rows; i++) { PREM.push_back(std::make_pair(LOADED_PREM[i][0], LOADED_PREM[i][1])); }" << endl;
         outfile1 << "  }\n\n\n\n\n\n" << endl;
